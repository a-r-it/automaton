...
| `tags` | string[] | no | Free-form tags (e.g. `["verification"]`). |
| `model` | string | no | Required `model` value for Agent dispatches during this task (e.g. `haiku`, `sonnet`, `opus`). Use when the task is sensitive to tier choice — empirical A/B measurements need a pinned model, coordinator quality calls need Opus, cheap bulk work needs Haiku. Overrides `modelTier`. |
| `modelTier` | "mechanical" \| "standard" \| "frontier" | no | Abstract capability tier for subagents executing this task. `"inherit"` as a mapping value means omit the Agent `model` param. A concrete `model` pin (above) always wins over the tier. Full design: README.md, Subagent Model Routing. |
