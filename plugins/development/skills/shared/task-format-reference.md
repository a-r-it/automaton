...
| `modelTier` | "mechanical" \| "standard" \| "frontier" | no | Abstract capability tier for subagents executing this task. Belongs to the opt-in model-routing flow: when the routing file exists (project `.automaton/development/model-routing.json`, else user-level `~/.automaton/development/model-routing.json`), plugin gates require it on every plan task (`pre-taskcreate-model-tier`) and enforce the resolved Agent `model` at dispatch (`pre-agent-model-routing`); the session-start notice carries the rules. File absent → gates are dormant, the key is inert metadata, and every subagent inherits the session model. `"inherit"` as a mapping value means omit the Agent `model` param. A concrete `model` pin (above) always wins over the tier. Full design: README.md, Subagent Model Routing. |
