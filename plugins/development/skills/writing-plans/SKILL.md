...
**You MUST NOT call `EnterPlanMode` or `ExitPlanMode` at any point during this skill.** This skill operates in normal mode and manages its own completion flow — it selects the execution method itself and invokes the execution skill directly. Calling `EnterPlanMode` traps the session in plan mode where Write/Edit are restricted. Calling `ExitPlanMode` breaks the workflow. If you feel the urge to call either, STOP — follow this skill's instructions instead.
...
**Context:** If working in an isolated worktree, it should have been created via the `development:using-git-worktrees` skill at execution time.

**Save plans to:** `docs/development/plans/YYYY-MM-DD-<feature-name>.md`
...
> **For agentic workers:** REQUIRED SUB-SKILL: Use development:subagent-driven-development (recommended) or development:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
<HARD-GATE>
STOP. DO NOT call EnterPlanMode or ExitPlanMode — EnterPlanMode traps the session (Write/Edit restricted); ExitPlanMode breaks the flow.
</HARD-GATE>

The plan is written and saved. **You choose the execution method yourself — do NOT ask the user** (unless the user explicitly asked to be involved in the choice). Pick based on the plan:

- **subagent-driven-development** (default) — the plan has ≥3 mostly-independent tasks, quality/review matters, or it's a long autonomous run. Fresh subagent per task + two-stage review; you coordinate in this session.
- **executing-plans** (inline) — the plan is small (1–2 tasks), or its tasks are tightly coupled so fresh-context-per-task adds nothing, or subagent overhead clearly exceeds the benefit. You execute the tasks directly in this session, in batches with checkpoints.

When genuinely unsure, default to subagent-driven-development.

<HARD-GATE>
Announce your choice in one line with a one-clause reason, then invoke the chosen skill via the Skill tool NOW. Do NOT implement tasks yourself directly — do NOT read files, make edits, or update task statuses outside the chosen skill.

- Subagent-Driven chosen → invoke `development:subagent-driven-development` (you stay in this session as coordinator)
- Inline chosen → invoke `development:executing-plans`
</HARD-GATE>
...
If the plan is saved to `docs/development/plans/2026-01-15-feature.md`, the tasks file MUST be saved to `docs/development/plans/2026-01-15-feature.md.tasks.json`.

```json
{
  "planPath": "docs/development/plans/2026-01-15-feature.md",
  "tasks": [
    {
      "id": 0,
      "subject": "Task 0: ...",
...
Both the plan `.md` and `.tasks.json` must be co-located in `docs/development/plans/`.

### Resuming Work

Any new session can resume by running:
```
/development:executing-plans <plan-path>
```
