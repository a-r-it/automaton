...
**Note:** This plugin produces significantly higher-quality work when it dispatches subagents. Prefer development:subagent-driven-development over this skill for same-session execution; use this skill when executing a plan in a separate session with review checkpoints.
...
3. If on main/master with no worktree: **REQUIRED SUB-SKILL:** Use `development:using-git-worktrees` to create one
...
- **REQUIRED SUB-SKILL:** Use development:finishing-a-development-branch
...
- **development:using-git-worktrees** - Ensures isolated workspace (creates one or verifies existing)
