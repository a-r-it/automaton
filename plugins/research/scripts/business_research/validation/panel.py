"""panel kind (V2 spec §5.1) — findings, data points, formulas."""
...
def formula_ids(formula: str) -> tuple[set[str] | None, str | None]:
    ...
    belong to the panel-kind validator, which has the full document to
    check against.
...
_PANEL_TOP_KEYS = {"schema_version", "panelist", "status", "summary", "findings",
                    "data_points", "sources", "limitations"}
...
# here rather than duplicated (sanctioned DAG: verification -> panel).
...
def _find_roster_entry(manifest: object, panelist: str) -> dict[str, object] | None:
    roster = manifest.get("roster") if isinstance(manifest, dict) else None
    if not isinstance(roster, list):
        return None
    for e in roster:
        if isinstance(e, dict) and e.get("id") == panelist:
            return e
    return None
...
def _find_formula_cycle(graph: dict[object, list[str]]) -> list[object] | None:
    ...
    already reported elsewhere as E-panel-data-point-id and this graph is
...
def validate_panel(doc: object, manifest: Mapping[str, object]) -> list[str]:
    if not isinstance(doc, dict):
        return ["E-panel-doc-type: panel response must be a single JSON object, "
                f"got {type(doc).__name__}"]

    errs: list[str] = []

    panelist = doc.get("panelist")
    entry = _find_roster_entry(manifest, panelist) if isinstance(panelist, str) else None
    if entry is None:
        errs.append(f"E-panel-unknown-panelist: {panelist!r} not found in manifest roster")

    actual_keys = set(doc.keys())
    missing_keys = _PANEL_TOP_KEYS - actual_keys
    extra_keys = actual_keys - _PANEL_TOP_KEYS
    if missing_keys or extra_keys:
        errs.append(f"E-panel-keys: missing {sorted(missing_keys)}, extra {sorted(extra_keys)}")

    if doc.get("schema_version") != "business-panel-v2":
        errs.append(f"E-panel-schema-version: expected 'business-panel-v2', "
                    f"got {doc.get('schema_version')!r}")

    if doc.get("status") != "complete":
        errs.append(f"E-panel-status: expected 'complete', got {doc.get('status')!r}")

    findings = doc.get("findings")
    if not isinstance(findings, list):
        errs.append(f"E-panel-findings: 'findings' missing or not a list, got {findings!r}")
        findings = []
    for i, f in enumerate(findings):
        if not isinstance(f, dict):
            errs.append(f"E-panel-finding-entry: findings[{i}] is not an object")
    finding_entries = [f for f in findings if isinstance(f, dict)]

    data_points = doc.get("data_points")
    if not isinstance(data_points, list):
        errs.append(f"E-panel-data-points: 'data_points' missing or not a list, "
                    f"got {data_points!r}")
        data_points = []
    for i, d in enumerate(data_points):
        if not isinstance(d, dict):
            errs.append(f"E-panel-data-point-entry: data_points[{i}] is not an object")
    dp_entries = [d for d in data_points if isinstance(d, dict)]

    sources = doc.get("sources")
    if not isinstance(sources, list):
        errs.append(f"E-panel-sources: 'sources' missing or not a list, got {sources!r}")
        sources = []
    for i, s in enumerate(sources):
        if not isinstance(s, dict):
            errs.append(f"E-panel-source-entry: sources[{i}] is not an object")
    ...
    for fid in f_ids:
        if not isinstance(fid, str) or not _F_ID_RE.match(fid):
            errs.append(f"E-panel-finding-id: {fid!r} does not match ^F[1-9][0-9]*$")
    dup = _dupes([i for i in f_ids if isinstance(i, str)])
    if dup:
        errs.append(f"E-panel-finding-id-duplicate: {dup}")

    d_ids = [d.get("id") for d in dp_entries]
    for did in d_ids:
        if not isinstance(did, str) or not _D_ID_RE.match(did):
            errs.append(f"E-panel-data-point-id: {did!r} does not match ^D[1-9][0-9]*$")
    dup = _dupes([i for i in d_ids if isinstance(i, str)])
    if dup:
        errs.append(f"E-panel-data-point-id-duplicate: {dup}")

    s_ids = [s.get("id") for s in source_entries]
    for sid in s_ids:
        if not isinstance(sid, str) or not _S_ID_RE.match(sid):
            errs.append(f"E-panel-source-id: {sid!r} does not match ^S[1-9][0-9]*$")
    dup = _dupes([i for i in s_ids if isinstance(i, str)])
    if dup:
        errs.append(f"E-panel-source-id-duplicate: {dup}")
    ...
    for f in finding_entries:
        ...
        if not isinstance(dpids, list):
            errs.append(f"E-panel-finding-data-point-ids: {fid!r} 'data_point_ids' "
                        f"missing or not a list")
            dpids = []
        for dpid in dpids:
            if not isinstance(dpid, str):
                errs.append(f"E-panel-finding-data-point-ids: finding {fid!r} 'data_point_ids' "
                            f"entry {dpid!r} is not a string")
            elif dpid not in d_id_set:
                errs.append(f"E-panel-ref-finding-datapoint: finding {fid!r} references "
                            f"unknown data point {dpid!r}")

        sids = f.get("source_ids")
        if not isinstance(sids, list):
            errs.append(f"E-panel-finding-source-ids: {fid!r} 'source_ids' missing or not a list")
            sids = []
        for sid in sids:
            if not isinstance(sid, str):
                errs.append(f"E-panel-finding-source-ids: finding {fid!r} 'source_ids' "
                            f"entry {sid!r} is not a string")
                continue
            src = sources_by_id.get(sid)
            if src is None:
                errs.append(f"E-panel-ref-finding-source: finding {fid!r} references "
                            f"unknown source {sid!r}")
            elif fid not in (src.get("supports_finding_ids") or []):
                errs.append(f"E-panel-ref-finding-source: source {sid!r} 'supports_finding_ids' "
    ...
    for s in source_entries:
        ...
        if not isinstance(sfids, list):
            errs.append(f"E-panel-source-supports-finding-ids: {sid!r} 'supports_finding_ids' "
                        f"missing or not a list")
            sfids = []
        for fid in sfids:
            if not isinstance(fid, str):
                errs.append(f"E-panel-source-supports-finding-ids: source {sid!r} "
                            f"'supports_finding_ids' entry {fid!r} is not a string")
                continue
            fnd = findings_by_id.get(fid)
            if fnd is None:
                errs.append(f"E-panel-ref-finding-source: source {sid!r} references "
                            f"unknown finding {fid!r}")
            elif sid not in (fnd.get("source_ids") or []):
                errs.append(f"E-panel-ref-finding-source: finding {fid!r} 'source_ids' "
                            f"missing back-reference to source {sid!r}")

        sdpids = s.get("supports_data_point_ids")
        if not isinstance(sdpids, list):
            errs.append(f"E-panel-source-supports-data-point-ids: {sid!r} "
                        f"'supports_data_point_ids' missing or not a list")
            sdpids = []
        for dpid in sdpids:
            if not isinstance(dpid, str):
                errs.append(f"E-panel-source-supports-data-point-ids: source {sid!r} "
                            f"'supports_data_point_ids' entry {dpid!r} is not a string")
                continue
            dp = dp_by_id.get(dpid)
            if dp is None:
                errs.append(f"E-panel-ref-datapoint-source: source {sid!r} references "
                            f"unknown data point {dpid!r}")
            elif dp.get("source_id") != sid:
                errs.append(f"E-panel-ref-datapoint-source: data point {dpid!r} 'source_id' "
    ...
    for d in dp_entries:
        ...
        if src is None:
            errs.append(f"E-panel-ref-datapoint-source: data point {did!r} references "
                        f"unknown source {dsid!r}")
        elif did not in (src.get("supports_data_point_ids") or []):
            errs.append(f"E-panel-ref-datapoint-source: source {dsid!r} 'supports_data_point_ids' "
    ...
    for d in dp_entries:
        ...
        if kind not in _DP_KINDS:
            errs.append(f"E-panel-data-point-kind: {did!r} kind {kind!r} not in {_DP_KINDS}")
        ...
        if not isinstance(metric, str) or not metric:
            errs.append(f"E-panel-data-point-metric: {did!r} missing non-empty 'metric'")

        unit = d.get("unit")
        if not isinstance(unit, str) or not unit:
            errs.append(f"E-panel-data-point-unit: {did!r} missing non-empty 'unit'")
        ...
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            errs.append(f"E-panel-data-point-value: {did!r} 'value' must be a non-boolean "
                        f"int/float, got {value!r}")
        elif not math.isfinite(value):
            errs.append(f"E-panel-data-point-value: {did!r} 'value' must be finite, "
                        f"got {value!r}")
        ...
        if kind in ("observed", "estimated"):
            if inputs != []:
                errs.append(f"E-panel-data-point-inputs: {did!r} kind={kind!r} requires "
                            f"inputs == [], got {inputs!r}")
            if formula != "":
                errs.append(f"E-panel-data-point-formula: {did!r} kind={kind!r} requires "
                            f"formula == '', got {formula!r}")
            sid = d.get("source_id")
            if not isinstance(sid, str) or not sid or sid not in sources_by_id:
                errs.append(f"E-panel-data-point-source: {did!r} kind={kind!r} requires a "
                            f"resolving 'source_id', got {sid!r}")

        elif kind == "calculated":
            if not isinstance(inputs, list) or not inputs:
                errs.append(f"E-panel-data-point-inputs: {did!r} kind='calculated' requires "
                            f"non-empty inputs, got {inputs!r}")
                inputs = []
            non_str_inputs = [i for i in inputs if not isinstance(i, str)]
            if non_str_inputs:
                errs.append(f"E-panel-data-point-inputs: {did!r} inputs entries "
            ...
            if bad_inputs:
                errs.append(f"E-panel-data-point-inputs: {did!r} inputs reference unknown "
                            f"data points {bad_inputs}")

            if not isinstance(formula, str) or not formula:
                errs.append(f"E-panel-data-point-formula: {did!r} kind='calculated' requires "
                ...
                if ferr is not None:
                    ...
                    if extra_refs:
                        errs.append(f"E-panel-data-point-formula-refs: {did!r} formula "
                                    f"references {sorted(extra_refs)} not in inputs {inputs}")
                    unused_inputs = set(str_inputs) - ids
                    if unused_inputs:
                        errs.append(f"E-panel-formula-unused-input: {did!r} inputs "
            ...
            if "source_id" in d and not isinstance(dp_source_id, str):
                errs.append(f"E-panel-data-point-source: {did!r} kind={kind!r} "
        ...
        if not isinstance(period, str) or not _PERIOD_RE.match(period):
            errs.append(f"E-panel-period: data point {did!r} period {period!r} invalid")
        ...
        if not isinstance(geography, str) or not (
            ...
            errs.append(f"E-panel-geography: data point {did!r} geography {geography!r} invalid")

    cycle = _find_formula_cycle(calc_graph)
    if cycle is not None:
        errs.append(f"E-panel-formula-cycle: cyclic calculated data point dependency {cycle}")
    ...
    # topic would silently make E-panel-required-topic pass or fail for the
    ...
    for f in finding_entries:
        ...
        if not isinstance(claim, str) or not claim:
            errs.append(f"E-panel-finding-claim: {fid!r} missing non-empty 'claim'")

        topic = f.get("topic")
        if not isinstance(topic, str) or not topic:
            errs.append(f"E-panel-finding-topic: {fid!r} missing non-empty 'topic'")

        confidence = f.get("confidence")
        if confidence not in _CONFIDENCE_VALUES:
            errs.append(f"E-panel-finding-confidence: {fid!r} confidence {confidence!r} "
                        f"not in {_CONFIDENCE_VALUES}")

    # --- doc-level summary: presence + type ---------------------------------
    summary = doc.get("summary")
    if not isinstance(summary, str) or not summary:
        errs.append("E-panel-summary: missing non-empty 'summary'")
    ...
    for f in finding_entries:
        ...
        if isinstance(claim, str) and has_numeric_token(claim) and not (
            ...
            errs.append(f"E-panel-numeric-claim: finding {fid!r} claim carries a numeric "
                        f"token but has no data_point_ids")

    if isinstance(summary, str) and has_numeric_token(summary) and len(dp_entries) == 0:
        errs.append("E-panel-numeric-summary: summary carries a numeric token but the "
        ...
        if isinstance(required_topics, list):
            ...
            for topic in required_topics:
                if topic not in finding_topics:
                    errs.append(f"E-panel-required-topic: no finding carries required "
                                f"topic {topic!r}")
    ...
    if entry is not None:
        ...
        if isinstance(caps, dict):
            if len(findings) > caps.get("findings", 0):
                errs.append(f"E-panel-cap-findings: {len(findings)} findings exceeds "
                            f"cap {caps.get('findings')}")
            if len(sources) > caps.get("sources", 0):
                errs.append(f"E-panel-cap-sources: {len(sources)} sources exceeds "
                            f"cap {caps.get('sources')}")
            if len(data_points) > caps.get("data_points", 0):
                errs.append(f"E-panel-cap-data-points: {len(data_points)} data_points exceeds "
                            f"cap {caps.get('data_points')}")
            byte_size = len(json.dumps(doc, ensure_ascii=False).encode())
            if byte_size > caps.get("bytes", 0):
                errs.append(f"E-panel-cap-bytes: {byte_size} bytes exceeds cap {caps.get('bytes')}")
    ...
    if not report_date_ok:
        errs.append("E-panel-manifest-report-date: manifest report_date malformed, "
                    "cannot compare accessed_at")
    for s in source_entries:
        sid = s.get("id")
        url = s.get("url")
        if not isinstance(url, str) or not url.startswith("https://"):
            errs.append(f"E-panel-source-url: source {sid!r} url {url!r} must start with https://")

        if not isinstance(s.get("title"), str) or not s.get("title"):
            errs.append(f"E-panel-source-title: source {sid!r} missing non-empty title")

        if not isinstance(s.get("publisher"), str) or not s.get("publisher"):
            errs.append(f"E-panel-source-publisher: source {sid!r} missing non-empty publisher")

        if not isinstance(s.get("usage"), str) or not s.get("usage"):
            errs.append(f"E-panel-source-usage: source {sid!r} missing non-empty usage")

        accessed_at = s.get("accessed_at")
        if not isinstance(accessed_at, str) or not _DATE_RE.match(accessed_at):
            errs.append(f"E-panel-source-date: source {sid!r} accessed_at {accessed_at!r} "
                        f"is not YYYY-MM-DD")
        elif (report_date_ok and isinstance(manifest_report_date, str)
                and accessed_at > manifest_report_date):
            errs.append(f"E-panel-source-date-future: source {sid!r} accessed_at "
