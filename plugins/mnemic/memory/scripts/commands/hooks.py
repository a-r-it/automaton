"""Toggle automation hooks marker for the mnemic plugin."""
...
def set_marker(root: Path, *, enabled: bool) -> bool:
    """Toggle enabled field in root/.automaton/mnemic/config.toml.
    ...
    try:
        ...
        raise errors.EnvError(
            ...
            hint="run `mnemic:wiki setup` to create .automaton/mnemic/config.toml",
        ) from None
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="hooks",
        description="Toggle mnemic hooks in .automaton/mnemic/config.toml",
        parents=[shared],
    )
    sub = parser.add_subparsers(dest="cmd")
    sub.add_parser(
        "enable", help="Set enabled=true in .automaton/mnemic/config.toml", parents=[shared]
    )
    sub.add_parser(
        "disable", help="Set enabled=false in .automaton/mnemic/config.toml", parents=[shared]
    )
